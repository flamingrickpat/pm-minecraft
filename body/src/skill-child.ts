...
function call(method: string, args: unknown, sync: boolean): unknown {
  const id = nextId++;
  if (!sync) {
  ...
  while (true) {
    if (readSync(3, byte, 0, 1, null) === 0) process.exit(0);
    if (byte[0] === 10) break;
    bytes.push(byte[0]);
  }
...
async function run(source: string, args: unknown, methods: string[]): Promise<void> {
  try {
    ...
    const api = new Proxy({}, {
      get(_target, property) {
        ...
        if (!methods.includes(property)) return () => {
          const error = new Error(`The survival API does not expose '${property}'.`);
          error.name = "AccessDenied";
          throw error;
        };
        return (value: unknown) => call(property, value, property === "findPath" || property === "findBlocks");
      },
    });
    const output = (stream: string) => (...values: unknown[]) => send({
    ...
    if (typeof skill !== "function") throw new Error("The skill has no default run export.");
    ...
    const value = error instanceof Object && "name" in error && "message" in error
      ? error as Error : { name: "Error", message: String(error) };
...
lines.on("line", (line) => {
  ...
  try { request.resolve(responseValue(message)); } catch (error) { request.reject(error as Error); }
});
