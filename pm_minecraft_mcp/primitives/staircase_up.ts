...
async function clearCellIfSolid(context: MinecraftContext, target: Vector3, cleared: string[]): Promise<void> {
  ...
  if (res.ok !== true && String(res.reason ?? "").includes("harvest")) {
    // A placeable block may be held for pillar_up; switch back to a pickaxe.
...
const run: SkillEntrypoint = async (context: MinecraftContext, rawInput: unknown) => {
  ...
  for (; guard < maxSteps; guard++) {
    ...
    // pillar_up places the HELD block under the bot, so switch from the
    // pickaxe to a placeable block before climbing.
    for (const block of ["cobblestone", "dirt", "cobbled_deepslate", "oak_planks", "birch_planks"]) {
      const e = await context.equip(block);
      if (e.ok === true) break;
    }
