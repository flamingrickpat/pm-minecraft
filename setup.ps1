...
$bodyPath = Join-Path $repoRoot "body"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        & npm install
        if ($LASTEXITCODE -ne 0) { throw "Could not install body Node dependencies." }
    }
    Push-Location $repoRoot
      try {
          & $pythonPath -m mcmcp.startup --patch-prismarine --body-root $bodyPath
      } finally {
          Pop-Location
      }
    if ($LASTEXITCODE -ne 0) { throw "Could not patch prismarine-viewer for Minecraft 1.19.4." }
    & npm run build
    if ($LASTEXITCODE -ne 0) { throw "Could not build the body." }
} finally {
    Pop-Location
}
...
if (-not (Test-Path -LiteralPath (Join-Path $bodyPath "dist\main.js") -PathType Leaf)) {
    throw "The body build output is missing: body\dist\main.js"
}
& node --version
